...
image = cv2.imread('./template1.png', 0)
image = image[0:-7, 5:-15]
image = img_as_float(image)
image *= 1/image.max()
# Feel free to play around with the parameters to see how they impact the result
cv = chan_vese(image, mu=0.005, lambda1=1, lambda2=1, tol=1e-3, max_iter=200,
               dt=0.5, init_level_set="small disk", extended_output=True)


cv = cv[0]
print(cv.shape)
edges = canny(cv, sigma=3)
result = hough_ellipse(edges, min_size=20, accuracy=25)
